...
// Google Drive's /uc?export=download URL no longer delivers raw
// bytes (Google intercepts large files with an HTML confirmation
// page), so passing it directly to Shopify always fails with
// "Invalid video url".  The correct approach:
//   1. Fetch file metadata from Drive (size + MIME type)
//   2. Ask Shopify for a pre-signed staged upload slot (GCS)
//   3. Download the video bytes from Drive (authenticated)
//   4. PUT those bytes to Shopify's GCS staged URL
//   5. Call productCreateMedia with the confirmed resourceUrl
app.post('/shopify-add-video', async (req, res) => {
  ...
  try {
    ...
    // 1. Get file metadata (size + MIME type) — needed for staged upload
    ...
    // 2. Create Shopify staged upload slot (PUT method — simpler than multipart POST)
    ...
    const stageRes = await fetch(`https://${SHOPIFY_STORE}/admin/api/2026-04/graphql.json`, {
      ...
      body: JSON.stringify({ query: stageMutation, variables: {
        input: [{ filename: name, mimeType, resource: 'VIDEO', fileSize: String(fileSize), httpMethod: 'PUT' }]
      }})
    });
    ...
    // 3. Download video bytes from Drive (authenticated — no public sharing needed)
    ...
    // 4. PUT bytes to Shopify's GCS staged URL
    const putHeaders = { 'Content-Type': mimeType, 'Content-Length': String(videoBuffer.length) };
    for (const p of (target.parameters || [])) { putHeaders[p.name] = p.value; }
    ...
    // 5. Attach confirmed video to the Shopify product
