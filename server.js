...
// ── Shopify: add video to product via productCreateMedia ──────
// Temporarily makes the Drive file public, hands the URL to
// Shopify's GraphQL productCreateMedia, then revokes access.
app.post('/shopify-add-video', async (req, res) => {
  ...
  try {
    const token = await getValidGoogleToken();
    // Make Drive file publicly readable
    let permId = null;
    try {
      ...
      permId = permData.id;
    } catch(e) { console.warn('Drive permission set warning:', e.message); }

    const sourceUrl = `https://drive.google.com/uc?export=download&id=${fileId}&confirm=t`;
    const gidProductId = productId.startsWith('gid://') ? productId : `gid://shopify/Product/${productId}`;

    const mutation = `mutation productCreateMedia($media:[CreateMediaInput!]!,$productId:ID!){productCreateMedia(media:$media,productId:$productId){media{alt mediaContentType status}mediaUserErrors{field message}}}`;
    const gqlRes = await fetch(`https://${SHOPIFY_STORE}/admin/api/2026-04/graphql.json`, {
      ...
      body: JSON.stringify({
        ...
        variables: {
          ...
          media: [{ originalSource: sourceUrl, alt: fileName || 'Product video', mediaContentType: 'VIDEO' }]
        }
      })
    });
    const gqlData = await gqlRes.json();
    const mediaErrors = gqlData?.data?.productCreateMedia?.mediaUserErrors || [];
    if (mediaErrors.length) return res.status(400).json({ error: mediaErrors.map(e=>e.message).join(', ') });

    // Revoke public access after 90 s — enough for Shopify to queue the download
    if (permId) {
      setTimeout(async () => {
        try {
          const tk = await getValidGoogleToken();
          await fetch(`https://www.googleapis.com/drive/v3/files/${fileId}/permissions/${permId}`, {
            method: 'DELETE', headers: { 'Authorization': 'Bearer ' + tk }
          });
          console.log('Drive permission revoked for', fileId);
        } catch(e) { console.warn('Permission revoke warning:', e.message); }
      }, 90000);
    }
    res.json({ success: true, media: gqlData?.data?.productCreateMedia?.media });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
