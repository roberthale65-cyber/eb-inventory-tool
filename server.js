...
app.post('/generate-description', async (req, res) => {
  ...
  const { eb_number, type, season, holiday, colors, dimensions,
          materials, location, special, notes, images } = req.body;

  const textPrompt = `You write product descriptions for Eternal Blooms Designs, a handmade artificial flower arrangement business in Omaha, Nebraska. You are writing in the voice of the artist and maker — first person, speaking directly to the customer. The tone is warm, passionate, and personal — like an artist who genuinely loves what she creates and is excited to share it. She is proud of her craft and wants the customer to feel the care and intention that went into each piece.

Write a product description for this listing in first person ("I", "my", "I made"). Keep it to 3–4 punchy sentences. No fluff, no generic filler. Make the reader feel the quality, the uniqueness, and the maker's passion. End with a one-of-a-kind callout that creates natural urgency — there is only one, and when it's gone it's gone.${images && images.length > 0 ? '\n\nPhoto(s) of the actual piece are attached — use what you can see in the photos alongside the notes below to write the most accurate and evocative description possible.' : ''}

Do NOT write in third person about the maker. Do NOT use phrases like "Patti made" or "created by". Write AS the maker, TO the customer.
...
Write ONLY the product description paragraph. No title, no bullet points, no intro phrase like "Here is..." — just the description starting directly with the voice of the maker.`;

  // Build message content — images first if provided, then text prompt
  let messageContent;
...
// ── Clean up spelling/grammar via Anthropic API ──────────────
  ...
  try {
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 200,
        messages: [{ role: 'user', content: `Fix any spelling and grammar errors in the text below. Keep the same meaning, tone, and style. Return ONLY the corrected text with no explanation, no quotes, no preamble.\n\n${text}` }]
      })
    });
    const data = await r.json();
    if (!r.ok) {
...
app.post('/create-product', async (req, res) => {
  ...
  try {
    ...
    const heroImageUrl = data.product.images && data.product.images.length > 0
